...
class SqlaOrdersRepository():
    ...
    async def update_info_on_paid(self, order: str, data: dict) -> None:
        async with self.session_factory() as session:
            ...
            street_line1 = data.get("street_line1") if data.get("street_line1") else None
            street_line2 = data.get("street_line2") if data.get("street_line2") else None
            post_code = data.get("post_code") if data.get("post_code") else None
            query = update(self.m).filter(self.m.order == order).values(
                {
                    "paid": True,
                    "name": name,
